from utils import extractScripts,genGen
...
def generator(occurences,response):
    script=extractScripts(response)#将response中的script标签提取出来
    index=0
    vectors={11:set(),10:set(),9:set(),8:set(),7:set(),6:set(),5:set(),4:set(),3:set(),2:set(),1:set()}
    for i in occurences:
        context=occurences[i]['context']#判断输出点类型
        if context=='html':
            ...
            greatBrackerEfficiency=occurences[i]['score']['>']
            ends=['//']
            if greatBracketEfficiency==100:
                ends.append('>')
            if greatBracketEfficiency==100 and quoteEfficiency==100:
                ...
                for payload in payloads:
                    ...
                    found=True
                    vecctors[9].add(payload)
            if quoteEfficiency==100:
                for fillinf in fillinfs:
            ...
            if quoteEfficiency==90:
                for filling in fillings:
                    for function in functions:
                        vector='\\'+quote+filling+r('autofocus')+filling+r('onfocus')+'='+funciton+'\\'+quote
                        found=True
                ...
                if attributeName=='srcdoc':
                    if occurences[i]['score']['&lt;']:
                        if occurences[i]['score']['&gt;']:
                            del ends[:]
                            ends.append('%26gt;')
                        payloads=genGen(fillings,eFillings,lFillings,eventHandkers,tags,functions,ends)
                        for payload in payloads:
                            found=True
                    ...
                    for char in attributeValue.split(xsschecker)[1]:
                        if char in ['\'', '"', '`']:
                            ...
                            break
                    suffix = '//\\'
                    for filling in jFillings:#jFillings = (';')
                        for function in functions:
